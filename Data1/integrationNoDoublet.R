...
nw_object <- c(26)
cluster26NBS <- subset(larvaOL.integrated, idents = nw_object)
cluster26NBS
saveRDS(cluster26NBS, "cluster26NBS.rds")

cluster26NBS <- readRDS("cluster26NBS.rds")
FeaturePlot(cluster26NBS , features = c('LOC6629478' , 'LOC6629707') , blend = TRUE )
FeaturePlot(cluster26NBS , features = 'rna_LOC6627282' )

gene <- "LOC6627282"
expr <- FetchData(cluster26NBS,vars = gene)
my_object <- cluster26NBS[, which(x = expr > 0)]
my_object
